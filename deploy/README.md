...
### Distribution de l'app (bêta)

La vitrine **ne sert plus d'APK** : l'app se distribue uniquement par le **test
fermé Google Play**. Les deux liens du parcours d'inscription (groupe de testeurs
puis opt-in) vivent dans `landing/src/config.ts`.

- `deploy.sh` extrait le `versionName` de `app/build.gradle.kts` vers
  `landing/src/version.json` : la vitrine annonce la version réellement publiée.
- Passage au Play Store public (NODE-147) : remplacer le parcours en deux étapes
  par le badge officiel.
